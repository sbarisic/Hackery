...
namespace Hackery {
	static class Magic {
		...
		public static byte[] Hook(IntPtr OldFunc, IntPtr NewFunc) {
			...
			using (MemoryManagement.Protect(OldFunc, JMP.Length, MemProtection.ExecReadWrite)) {
		...
		public static HookHandle Hook(MethodInfo OldFunc, IntPtr NewFunc) {
			return new HookHandle(OldFunc, Hook(GetNativePointer(OldFunc), NewFunc));
		}

		public static HookHandle Hook(MethodInfo OldFunc, MethodInfo NewFunc) {
			return Hook(OldFunc, GetNativePointer(NewFunc));
		}
		...
		public static HookHandle Hook<THooked, THook>(THooked old, THook @new) {
			...
			if (oldParameters.Count != newParameters.Count)
				throw new ArgumentException("Parameter count (eventually including initial \"this\" parameter) mismatch: Tried to hook method with " + oldParameters.Count + " with one with " + newParameters.Count + " parameters.");
